...
import './style.css'
import { Edge, ForceDirectedGraph, Graph, Offset } from 'graphview-typescript';
...
algorithm.setDimensions(rect.width, rect.height);
algorithm.init(graph);

const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
svg.setAttribute('width', algorithm.graphWidth.toString());
svg.setAttribute('height', algorithm.graphHeight.toString());
...
setInterval(async () => {
  ...
  graph.edges.forEach((edge) => {
    ...
    const focusedNode = algorithm.focusedNode;
    if (focusedNode) {
    ...
    if (delta > tickMs) {
      console.warn(`Tick took ${delta}ms`);
    }
    algorithm.copyPositions(graph);
    animating = false;
  });
